...
export default class ListItem extends Component {
	...
	render(){
		...
		const position_name = Utils.formatNone(data.position_name);
		return(
			<tr>
				{
				...
				<td>{Utils.formatNone(data.brand_name)}</td>
				...
				<td>{Utils.formatNone(data.commission)}</td>
				{
				...
				<td>{Utils.DateFormatToDays(data.end_time)}</td>
			</tr>
		)
	}
}