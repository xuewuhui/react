...
class Pagination extends Component {
	...
	caclePages() {
		...
		Utils.range(viewPageStart, viewPageEnd + 1).map((index, key) => {
			let _className = '';
			if(index === currentPage) {
				_className = 'ms-pagination-current';
			}
			return ret.push(<li onClick={this.handlePageClick.bind(this, index)} key={'pagination-' + key} className={_className}>{index}</li>);
		});
		...
		return (
			<div className="ms-pagination">
				...
				<div className="ms-pagination-pages">
					共 <span style={{color:'red'}}>{this.props.totalNumber}</span> 个
				</div>
