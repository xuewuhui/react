...
// this.key=this.escape(this.key); // not req
	...
	for(var m=0;m<18;m+=2){
		this.encipher();
		this.bf_P[m]=this.xl_par;
		this.bf_P[m+1]=this.xr_par;
	}
	for(j=0;j<256;j+=2){
		this.encipher();
...
{
var l = this.unescape(t.substr(w, 8));
var r = this.unescape(t.substr(w + 8, 8));
...
Blowfish.prototype.wordescape=function(w){
	var r='';
	var m = [this.wordbyte3(w),this.wordbyte2(w),this.wordbyte1(w),this.wordbyte0(w)];
	for(var i=3;i>=0;i--){
...
Blowfish.prototype.pkcs7_encode = function(text, block_size) {
...
Blowfish.prototype.encrypt_with_pkcs7_bf = function(key, text) {
	var bf = new Blowfish(key);
	return bf.encrypt(this.pkcs7_encode(text, 8));
}
